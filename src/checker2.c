...
void undo(int k, int redmoves, int bluemoves, struct move storered[100000], struct move storeblue[100000], int board[10][10]) // k represents the number of moves the player wants to undo
    ...
    retry:
        ...
        if (c1 == 'X' && y1 == 0 && c2 == 'X' && y2 == 0) // If thre user enters X0 X0 then they get access to the functions
        {
            ...
            else if (ch == 'U') // To undo moves,here u can undo any no of moves,user needs to enter the number of undos they want to do
            {
                ...
                if (p1 == 1 && p2 == 1)
                {
                    undo(num_moves, redmoves, bluemoves, storered, storeblue,arr);
                }
                else
                {
                    printf("\n");
